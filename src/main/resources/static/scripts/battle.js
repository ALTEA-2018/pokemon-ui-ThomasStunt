    ...
    await $.ajax(
        {
            ...
            success: function(data) {
                console.log(data);
                battle = data;
                trainerCurrentPokemon = data.trainer.team[0];
                opponentCurrentPokemon = data.opponent.team[0];
            }
        })
...
async function enterPokemon(trainerName, pokemon, front){
    ...
    const pokemonSprite = front ? pokemon.pokemonType.sprites.front_default : pokemon.pokemonType.sprites.back_default;
...
async function sendAttack(trainerName){
    const result = await $.ajax(
        {
            type: 'POST',
            url: `/api/battles/${battle.uuid}/${trainerName}/attack`,
            dataType: 'json'
        });
